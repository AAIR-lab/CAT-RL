...
if __name__=="__main__":
    ...
    map_names = {"gridworld": "grid_64x64_map1", "taxiworld": "taxi_30x30_map1", "officeworld": "office_36x36_map1", "waterworld": "water_"+str(gridsize[0])+"x"+str(gridsize[1])+"_map1", "lunarlander": "lunarlander", "cartpole": "cartpole", "mountaincar":"mountaincar"}
        ...
        for exp in exps:
          for alg in algorithms:
            ...
            if alg == "dqn":
              ...
              #         learning_rate= 4e-3, learning_starts=1000, buffer_size= 100000, batch_size= 128, \
              #         target_update_interval= 600, train_freq= 16, gradient_steps= 8, exploration_fraction= 0.2, exploration_final_eps= 0.07)
              # model = DQN('MlpPolicy', envs, verbose=verbose, gamma=gamma, policy_kwargs={"net_arch":[256,256]}, \
              ...
              model = A2C('MlpPolicy', envs, verbose=verbose, gamma=gamma, policy_kwargs={"net_arch":[256,256]}, \
                      learning_rate= 1e-4, n_steps= 64, use_sde=True, sde_sample_freq=10)
            elif alg == "ppo":
              gamma = 0.99
              model = PPO('MlpPolicy', envs, verbose=verbose, gamma=gamma, policy_kwargs={"net_arch":[256,256]}, \
                      learning_rate= 1e-4, batch_size= 64, use_sde=True, sde_sample_freq=-1, gae_lambda= 0.98, n_steps= 16)
