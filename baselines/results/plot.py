...
def moving_average (y, moving_number):
    y_m = []
    for i in range (moving_number, len(y)):#len(y)):
        sum_temp = 0
...
def read_data (map_name, approach_name, exp_number):
    ...
    for i in range (exp_number):
        path = p + "/mountaincar/dqn/train/" + map_name + "_" + approach_name + "_" + str(i + 1)
...
def compare_with_bounds(methods, map_name, exp_number, param, moving_number):
    colors = {'dqn': ['#45AA99','#D0E9E5'], 'adrl-agg': ['#f56f0f','#edb68e'],'ppo': ['#332288','#CBC7E1'], 'hrl': ['#CD6A7B','#F2D8DC'], 'jirp': ['#BFB113','#FFF8A7']}
    for m in methods:
    ...
    p = os.getcwd()
    path = p + "/mountaincar/dqn/train/" + map_name + "_" + "dqn" + "_" + str(exp_number)
...
def read_data_single (map_name, approach_name, exp_number):
    data = []
    p = os.getcwd()
    path = p + "/mountaincar/dqn/train/" + map_name + "_" + approach_name + "_" + str(exp_number)
    with open(path, "rb") as file:
        data.append( pk.load(file) )
    return data
...
def plot_single (map_name, approach_name, exp_number, moving_number, param):
    ...
    p = os.getcwd()
    path = p + "/baselines/results/mountaincar/dqn/train/" + map_name + "_" + approach_name + "_" + str(exp_number)
    plt.savefig(path+".png")
    # plt.show()
...
#plot_single ('water_300x300', 'adrl', 9, smoothing, 'success')
