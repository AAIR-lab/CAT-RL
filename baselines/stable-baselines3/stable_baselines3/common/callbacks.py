...
class CustomBaseCallback(BaseCallback):

    def on_step(self):
        flag = False
        for i in range(len(self.locals["infos"])):
            if self.locals["infos"][i]['done']:
                self.n_calls += 1
                flag = True
        if flag:
            return self._on_step()
...
class CustomEventCallback(CustomBaseCallback):
    """
    Base class for triggering callback on event.
...
class EvalCallback(CustomEventCallback):
    """
    ...
    def __init__(
        ...
        if log_path is not None:
            log_path = os.path.join(log_path, "evaluations{}".format(runid))
    ...
    def _on_step(self) -> bool:
        ...
        # print(self.n_calls)
        if self.n_calls > 0 and self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            if self.log_path is not None:
                self.num_episodes.append(self.n_calls)
